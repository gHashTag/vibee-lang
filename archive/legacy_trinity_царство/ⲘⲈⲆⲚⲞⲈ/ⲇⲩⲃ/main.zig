...
const tri_parser = @import("tls_parser.zig");
...
pub fn main() !u8 {
    ...
    if (args.len < 2) {
        try stdout.writeAll("Usage: trinity <command> [options]\n\n");
        try stdout.writeAll("Commands:\n");
        try stdout.writeAll("  gen <file.tri>  Generate code from TRI specification\n");
    ...
    if (std.mem.eql(u8, command, "version")) {
        try stdout.print("Trinity version {s}\n", .{VERSION});
        return 0;
    }
    ...
    if (std.mem.eql(u8, command, "gen")) {
        ...
        // Parse
        var parser = tri_parser.TLSParser.init(allocator);
        defer parser.deinit();
        ...
        // Write output
        const output_path = "trinity/output/trinity_vm_generated.zig";
