...
Версия без внешних зависимостей (только стандартная библиотека)
...
print("\n2.1 Связь показателя 3/2 с золотой идентичностью")
exponent = 3/2
...
print(f"f(τ) = {bgk_relaxation(f0, f_eq, tau, tau):.4f} (ожидается {f_eq + (f0-f_eq)/E:.4f})")
print(f"f(5τ) = {bgk_relaxation(f0, f_eq, tau, 5*tau):.4f} (ожидается ≈ {f_eq:.4f})")
