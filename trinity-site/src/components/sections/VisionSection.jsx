  ...
  return (
    <Section id="vision">
      ...
      <div className="fade" style={{ marginTop: '3rem', display: 'flex', gap: '4rem', justifyContent: 'center', flexWrap: 'wrap', textAlign: 'left' }}>
        <div>
          <h4 style={{ color: 'var(--accent)', marginBottom: '1rem', fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '0.1em' }}>2032 Goals</h4>
          {v.goals?.map((g, i) => (
            <div key={i} style={{ fontSize: '0.9rem', color: 'var(--muted)', marginBottom: '0.5rem' }}>• {g}</div>
          ))}
        </div>
        <div>
          <h4 style={{ color: 'var(--accent)', marginBottom: '1rem', fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Why Invest Now</h4>
          {v.whyNow?.map((w, i) => (
